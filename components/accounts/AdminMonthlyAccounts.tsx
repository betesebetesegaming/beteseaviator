...
export function AdminMonthlyAccounts() {
  ...
  const totals = useMemo(() => {
    ...
    return rows.reduce(
      ...
      { sales: 0, profit: 0, deposits: 0, withdrawals: 0, providerDue: 0, agentCommission: 0 }
    );
  }, [rows]);
  ...
  return (
    <>
      <Card className="mb-6 border-emerald-500/20 bg-emerald-500/5 p-5">
        ...
        <ol className="grid gap-2 text-sm text-slate-300 sm:grid-cols-2 lg:grid-cols-4">
          <li>
            <span className="font-medium text-white">1. Customer money</span>
            <p className="text-xs text-slate-500">Deposits in · withdrawals out (ModemPay + cash desk)</p>
          </li>
          <li>
            <span className="font-medium text-white">2. Sales</span>
            <p className="text-xs text-slate-500">What players bet minus what they won (GGR)</p>
          </li>
          <li>
            <span className="font-medium text-white">3. Vendors</span>
            <p className="text-xs text-slate-500">
              {providerName} ({providerPct}% of sales) + agents ({agentPct}%)
            </p>
          </li>
          <li>
            <span className="font-medium text-white">4. Your profit</span>
        ...
        <Card className="p-5">
          {selected ? (
            <>
              ...
              <BreakdownRow
                label="Deposits"
                hint="Money customers put in"
                value={formatXof(selected.deposits)}
              />
              <BreakdownRow
                label="Withdrawals"
                hint="Money customers took out"
